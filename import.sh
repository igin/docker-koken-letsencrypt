...
echo "DROP DATABASE IF EXISTS ${MYSQL_DATABASE};" | docker exec -i ${DB_SERVER} /usr/bin/mysql -u root --password="${MYSQL_ROOT_PASSWORD}"
echo "CREATE DATABASE ${MYSQL_DATABASE};" | docker exec -i ${DB_SERVER} /usr/bin/mysql -u root --password="${MYSQL_ROOT_PASSWORD}"
